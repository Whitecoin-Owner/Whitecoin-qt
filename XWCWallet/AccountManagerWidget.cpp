...
void AccountManagerWidget::deleteButtonSlots()
{
    ...
    connect(wi,&PasswordConfirmWidget::confirmSignal,[this,accountName,row](){
        ...
        qDebug()<<"clicked          "<<row;
        XWCWallet::getInstance()->postRPC( "id-remove_local_account",
...
void AccountManagerWidget::jsonDataUpdated(QString id)
{
    if("id-remove_local_account" == id)
    {
        ...
        XWCWallet::getInstance()->postRPC( "id-refresh-account",
...
void AccountManagerWidget::InitWidget()
{
    ...
    //初始化账户信息
    int totalPage = (XWCWallet::getInstance()->accountInfoMap.size()%3 == 0 &&
                     !XWCWallet::getInstance()->accountInfoMap.isEmpty())?
                    XWCWallet::getInstance()->accountInfoMap.size()/3 :
                    XWCWallet::getInstance()->accountInfoMap.size()/3 + 1;
    ...
    RefreshTableShow(0);
    _p->pageWidget->setShowTip(XWCWallet::getInstance()->accountInfoMap.size(),3);
    _p->pageWidget->setVisible(0 != XWCWallet::getInstance()->accountInfoMap.size());
    ...
    connect( XWCWallet::getInstance(), &XWCWallet::jsonDataUpdated, this, &AccountManagerWidget::jsonDataUpdated);
