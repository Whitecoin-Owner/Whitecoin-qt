...
void NameDialog::on_nameLineEdit_textChanged(const QString &arg1)
{
    ...
    //检查地址名是否在钱包内已经存在
    if( XWCWallet::getInstance()->accountInfoMap.keys().contains(addrName))
    {
        ui->okBtn->setEnabled(false);
    ...
    XWCWallet::getInstance()->postRPC( "id-get_account-" + addrName, toJsonFormat( "get_account", QJsonArray() << addrName ));
    ui->gifLabel->show();
}
