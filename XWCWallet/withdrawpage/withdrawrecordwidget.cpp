...
//    ui->withdrawRecordTableWidget->setColumnWidth(5,70);
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->withdrawRecordTableWidget);
    init();
}
...
void WithdrawRecordWidget::init()
{
    ...
    {
        if(ASSET_NAME == XWCWallet::getInstance()->assetInfoMap.value(assetId).symbol) continue;
...
void WithdrawRecordWidget::showWithdrawRecord(QString _accountAddress, QString _assetId)
{
    ...
    TransactionTypeIds typeIds = XWCWallet::getInstance()->transactionDB.getAccountTransactionTypeIdsByType(_accountAddress,TRANSACTION_TYPE_WITHDRAW);
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        AssetInfo assetInfo = XWCWallet::getInstance()->assetInfoMap.value(amountAssetId);

        if(_assetId != "ALL"  &&  amountAssetId != _assetId)    continue;


        ui->withdrawRecordTableWidget->setRowCount(rowCount + 1);
        ui->withdrawRecordTableWidget->setRowHeight(rowCount,42);
        ...
        AssetInfo feeAssetInfo = XWCWallet::getInstance()->assetInfoMap.value(feeAssetId);
