...
public:
        ...
        :nonceNumber(-1)
    {
        ...
        {
            crossFilePath = XWCWallet::getInstance()->configFile->value("/settings/chainPath").toString() + "/crossmark.dat";
        }
        else
        {
            crossFilePath = XWCWallet::getInstance()->appDataPath + "/crossmark.dat";
        }
    }
public:
    ...
    :_p(new DataPrivate())
{
    ...
    connect( XWCWallet::getInstance(), &XWCWallet::jsonDataUpdated, this, &CrossCapitalMark::jsonDataUpdated);
...
void CrossCapitalMark::httpReplied(QByteArray _data, int _status)
{
    ...
    if(object.value("chainId").toString().contains("erc")||object.value("chainId").toString().contains("eth"))
    {
...
void CrossCapitalMark::QueryETHHeight(const QString &symbol)
{
    int id = 0;
    if(symbol.contains("erc",Qt::CaseInsensitive)||symbol.contains("eth",Qt::CaseInsensitive))
    {
    ...
    paramObject.insert("chainId",symbol);
    object.insert("params",paramObject);
    _p->ethHeightHttp.post(XWCWallet::getInstance()->middlewarePath,QJsonDocument(object).toJson());
...
void CrossCapitalMark::TransactionInput(const QString &jsonString,const QString &symbol,const QString &accountName,double number)
{
    ...
    //解锁交易id
    XWCWallet::getInstance()->postRPC( "mark_decoderawtransaction+"+accountName+"+"+symbol+"+"+QString::number(number),
