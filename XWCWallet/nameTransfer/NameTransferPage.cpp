    ...
    ui(new Ui::NameTransferPage)
{
    ...
    ui->feeLabel->setText(QString::number(XWCWallet::getInstance()->feeChargeInfo.transferFee.toDouble(),'f',ASSET_PRECISION) + " " + ASSET_NAME);
}
...
void NameTransferPage::jsonDataUpdated(QString id)
{
    if( id == "NameTransferPage-decode_multisig_transaction-" + ui->trxCodeLineEdit->text())
    {
        ...
        if(result.startsWith("\"result\"") && !result.startsWith("\"result\":{\"ref_block_num\":0"))
        {
            ...
            if(XWCWallet::getInstance()->isMyAddress(to))
            {
            ...
            XWCWallet::getInstance()->postRPC( "NameTransferPage-get_account_by_addr-" + ui->trxCodeLineEdit->text(), toJsonFormat( "get_account_by_addr", QJsonArray() << from));
        }
        else
        {
            ui->widget->hide();
    ...
    if( id == "NameTransferPage-confirm_name_transfer")
    {
        ...
        {
            XWCWallet::getInstance()->postRPC( "NameTransferPage-info", toJsonFormat( "info", QJsonArray()));
        }
        else
        {
    ...
    if( id.startsWith("NameTransferPage-get_transaction-"))
    {
        ...
        if(result == "\"result\":null")
        {
            if(ui->trxTableWidget->item(row,5) && ui->trxTableWidget->item(row,4))
            {
                ...
                QString trxStr = XWCWallet::getInstance()->transactionDB.getNameTransferTrx(trxCode).at(0);
...
{
    QStringList trxCodeList = XWCWallet::getInstance()->transactionDB.getNameTransferTrxsCode();
    ...
    {
        ui->trxTableWidget->setRowHeight(i,42);

        QString trxCode = trxCodeList.at(i);
        QStringList trxStrList = XWCWallet::getInstance()->transactionDB.getNameTransferTrx(trxCode);
        ...
        XWCWallet::getInstance()->postRPC( "NameTransferPage-get_transaction-" + QString::number(i), toJsonFormat( "get_transaction", QJsonArray() << trxStrList.at(1)));
    }
...
void NameTransferPage::on_trxTableWidget_cellClicked(int row, int column)
{
    ...
    if(column == 6)
    {
        ...
        QString trxCode = ui->trxTableWidget->item(row, 4)->data(Qt::UserRole).toString();
        XWCWallet::getInstance()->transactionDB.removeNameTransferTrx(trxCode);
        showNameTransferTrxs();
...
{
    XWCWallet::getInstance()->postRPC( "NameTransferPage-decode_multisig_transaction-" + arg1, toJsonFormat( "decode_multisig_transaction", QJsonArray() << arg1));
}
...
void NameTransferPage::on_signBtn_clicked()
{
    ...
    TransferConfirmDialog transferConfirmDialog( ui->fromAddressLabel->text(), amountStr,
                                                 ...
                                                 QString::number(XWCWallet::getInstance()->feeChargeInfo.transferFee.toDouble(),'f',ASSET_PRECISION), ASSET_NAME, "");
    bool yOrN = transferConfirmDialog.pop();
    if(yOrN)
    {
        XWCWallet::getInstance()->postRPC( "NameTransferPage-confirm_name_transfer", toJsonFormat( "confirm_name_transfer",
                                                                                                 QJsonArray() << XWCWallet::getInstance()->addressToName(ui->sendtoLabel->text())
