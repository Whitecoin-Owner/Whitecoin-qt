...
void ExchangeMyOrdersWidget::jsonDataUpdated(QString id)
{
    ...
    if( id.startsWith("ExchangeMyOrdersWidget+invoke_contract_testing+cancelOrder+"))
    {
        ...
        ExchangeContractFeeDialog exchangeContractFeeDialog(totalAmount, XWCWallet::getInstance()->currentAccount);
        if(exchangeContractFeeDialog.pop())
        {
            ...
            XWCWallet::getInstance()->postRPC( "ExchangeMyOrdersWidget+invoke_contract+cancelOrder",
                                             toJsonFormat( "invoke_contract",
                                                           QJsonArray() << XWCWallet::getInstance()->currentAccount << XWCWallet::getInstance()->currentContractFee()
    ...
    if( id == "ExchangeMyOrdersWidget+invoke_contract+cancelOrder")
    {
        ...
        getUserOrders(XWCWallet::getInstance()->currentExchangePair);
...
{
    AccountInfo accountInfo = XWCWallet::getInstance()->accountInfoMap.value( XWCWallet::getInstance()->currentAccount);

    QString params = QString("%1,%2,%3,%4").arg(_pair.first).arg(_pair.second).arg(accountInfo.address).arg( "sell");
    XWCWallet::getInstance()->postRPC( "ExchangeMyOrdersWidget+invoke_contract_offline+getUserOrders+sell+" + _pair.first + "+" + _pair.second,
                                     toJsonFormat( "invoke_contract_offline",
                                     QJsonArray() << XWCWallet::getInstance()->currentAccount << EXCHANGE_MODE_CONTRACT_ADDRESS
                                                   << "getUserOrders"  << params));

    params = QString("%1,%2,%3,%4").arg(_pair.first).arg(_pair.second).arg(accountInfo.address).arg( "buy");
    XWCWallet::getInstance()->postRPC( "ExchangeMyOrdersWidget+invoke_contract_offline+getUserOrders+buy+" + _pair.first + "+" + _pair.second,
                                     toJsonFormat( "invoke_contract_offline",
                                     QJsonArray() << XWCWallet::getInstance()->currentAccount << EXCHANGE_MODE_CONTRACT_ADDRESS
                                                   << "getUserOrders"  << params));
}
...
void ExchangeMyOrdersWidget::showMyOrders()
{
    ...
    {
        ui->myOrdersTableWidget->setRowHeight(i,42);

        const OrderInfo& orderInfo = mySellOrdersMap.value( mySellOrdersMap.keys().at(i));

        TransactionStruct ts = XWCWallet::getInstance()->transactionDB.getTransactionStruct( orderInfo.trxId);
        QString trxTime;
        ...
        QTableWidgetItem* item = new QTableWidgetItem( QString::number(price,'f', XWCWallet::getInstance()->getExchangePairPrecision(XWCWallet::getInstance()->currentExchangePair))
                                                       + " / "
                                                       + ((orderInfo.completedBaseAmount == 0) ? "-" : QString::number(completedPrice,'f', XWCWallet::getInstance()->getExchangePairPrecision(XWCWallet::getInstance()->currentExchangePair)))
                                                       );
...
void ExchangeMyOrdersWidget::on_myOrdersTableWidget_cellClicked(int row, int column)
{
    if(column == 4)
    {
        if( ui->myOrdersTableWidget->item(row,1))
        {
            ...
            if(commonDialog.pop())
            {
                QString params = QString("%1,%2,%3,%4").arg(XWCWallet::getInstance()->currentExchangePair.first)
                        .arg(XWCWallet::getInstance()->currentExchangePair.second)
                        .arg(orderInfo.trxId)
                        .arg(type);
//                XWCWallet::getInstance()->postRPC( "ExchangeMyOrdersWidget+invoke_contract+cancelOrder",
//                                                 toJsonFormat( "invoke_contract",
//                                                 QJsonArray() << XWCWallet::getInstance()->currentAccount << "0.00001" << 100000 << EXCHANGE_MODE_CONTRACT_ADDRESS
//                                                               << "cancelOrder"  << params));
                XWCWallet::getInstance()->postRPC( "ExchangeMyOrdersWidget+invoke_contract_testing+cancelOrder+" + params,
...
void ExchangeMyOrdersWidget::queryOrders(int page, int pageCount, QString id)
{
    ...
    object.insert("id",id);
    object.insert("method","xwc.fdxqs.exchange.order.query");
...
void ExchangeMyOrdersWidget::showHistoryOrders()
{
    ...
    {
        ui->historyOrdersTableWidget->setRowHeight(i,42);
...
//        dateTime = dateTime.addSecs( -600);
        ...
        QTableWidgetItem* item = new QTableWidgetItem( QString::number(price,'f', XWCWallet::getInstance()->getExchangePairPrecision(XWCWallet::getInstance()->currentExchangePair))
                                                       + " / "
                                                       + ((completedBaseAmount == 0) ? "-" : QString::number(completedPrice,'f', XWCWallet::getInstance()->getExchangePairPrecision(XWCWallet::getInstance()->currentExchangePair)))
                                                       );
        item->setTextColor(QColor(90,115,227));
...
//        AssetInfo sellAssetInfo = XWCWallet::getInstance()->assetInfoMap.value(XWCWallet::getInstance()->getAssetId(sellSymbol));
//        AssetInfo buyAssetInfo  = XWCWallet::getInstance()->assetInfoMap.value(XWCWallet::getInstance()->getAssetId(buySymbol));
...
//            if(XWCWallet::getInstance()->getExchangeContractAddress(ui->accountComboBox->currentText()) == contractAddress)
//            {
