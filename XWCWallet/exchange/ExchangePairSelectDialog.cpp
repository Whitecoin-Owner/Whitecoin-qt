...
void ExchangePairSelectDialog::showPairs()
{
    if(quoteAssetSymbol.isEmpty())
    {
        QList<ExchangePair> pairs = XWCWallet::getInstance()->getExchangePairsByQuoteAsset();

        XWCWallet::getInstance()->configFile->beginGroup("/MyExchangePairs");
    ...
    {
        QList<ExchangePair> pairs = XWCWallet::getInstance()->getExchangePairsByQuoteAsset(quoteAssetSymbol);
