...
void BuyOrderWidget::setOrderInfo(unsigned long long _buyAmount, QString _buySymbol, unsigned long long _sellAmount, QString _sellSymbol)
{
    ...
    AssetInfo buyAssetInfo  = XWCWallet::getInstance()->assetInfoMap.value(XWCWallet::getInstance()->getAssetId(getRealAssetSymbol( buySymbol)));
    AssetInfo sellAssetInfo = XWCWallet::getInstance()->assetInfoMap.value(XWCWallet::getInstance()->getAssetId(getRealAssetSymbol( sellSymbol)));
    ...
    ui->amountLineEdit->setPlaceholderText(tr("Max: %1 %2").arg(XWCWallet::getInstance()->getAccountBalance(accountName, revertERCSymbol( sellSymbol))).arg(revertERCSymbol( sellSymbol)));
...
void BuyOrderWidget::estimateContractFee()
{
    ...
    XWCWallet::getInstance()->postRPC( "id-transfer_to_contract_testing-BuyOrderWidget", toJsonFormat( "transfer_to_contract_testing",
...
void BuyOrderWidget::on_okBtn_clicked()
{
    ...
    XWCWallet::getInstance()->postRPC( "id-transfer_to_contract-BuyOrderWidget", toJsonFormat( "transfer_to_contract",
                                                                           QJsonArray() << accountName << ui->contractAddressLabel->text()
                                                                           << ui->amountLineEdit->text() << getRealAssetSymbol( sellSymbol)
                                                                           << QString("%1,%2").arg(getRealAssetSymbol( buySymbol)).arg(decimalToIntegerStr(QString::number(amount,'g',8),buyAssetInfo.precision))
...
{
    ui->amountLineEdit->setText(XWCWallet::getInstance()->getAccountBalance(accountName,getRealAssetSymbol( sellSymbol)));
}
...
{
    if(ui->amountLineEdit->text().toDouble() > XWCWallet::getInstance()->getAccountBalance(accountName,getRealAssetSymbol( sellSymbol)).toDouble())
    {
        ui->amountLineEdit->setText(XWCWallet::getInstance()->getAccountBalance(accountName,getRealAssetSymbol( sellSymbol)));
    }
}
