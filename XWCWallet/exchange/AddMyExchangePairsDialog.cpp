...
void AddMyExchangePairsDialog::showPairsByQuoteAsset(QString quoteAsset)
{
    ...
    QList<ExchangePair> pairs = XWCWallet::getInstance()->getExchangePairsByQuoteAsset(quoteAsset);
    int count = 0;
    foreach (const ExchangePair& pair, pairs)
    {
        ...
        w->setPair(pair);
        w->setChecked( XWCWallet::getInstance()->isMyFavoritePair(pair));
...
void AddMyExchangePairsDialog::onCheckStateChanged(ExchangePair pair, bool isChecked)
{
    ...
    if(isChecked)
    {
        XWCWallet::getInstance()->configFile->setValue("/MyExchangePairs/" + str, 1);
    }
    else
    {
        XWCWallet::getInstance()->configFile->remove("/MyExchangePairs/" + str);
    }
}
