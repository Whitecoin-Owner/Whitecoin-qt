...
//    ui->ordersTableWidget->horizontalHeader()->setFixedHeight(35);
    ...
    favoritePairsWidget->move(60,100);
...
void OnchainOrderPage::init()
{
    ...
    if(XWCWallet::getInstance()->currentSellAssetId.isEmpty() || XWCWallet::getInstance()->currentBuyAssetId.isEmpty())
    {
        XWCWallet::getInstance()->configFile->beginGroup("/FavoriteOrderPairs");
...
void OnchainOrderPage::on_favoriteBtn_clicked()
{
    ...
    if(ui->favoriteBtn->isChecked())
    {
        ...
        XWCWallet::getInstance()->configFile->setValue("/FavoriteOrderPairs/" + str, 1);
        ui->favoriteBtn->setChecked(true);

    }
    else
    {
        XWCWallet::getInstance()->configFile->remove("/FavoriteOrderPairs/" + str);
        ui->favoriteBtn->setChecked(false);
    }
