        ...
        if(result.startsWith(QString("\"result\":")))
        {
            ...
            if(isStandard)
            {
                if(codeHashList.contains(codeHash))
                {
                    ...
                    XWCWallet::getInstance()->postRPC( "AddTokenDialog-invoke_contract_offline-state-" + contractId, toJsonFormat( "invoke_contract_offline",
...
void AddTokenDialog::on_okBtn_clicked()
{
    ...
    {
        XWCWallet::getInstance()->postRPC( "AddTokenDialog-get_contract_info", toJsonFormat( "get_contract_info",
