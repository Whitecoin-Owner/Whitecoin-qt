...
void ProposalPage::init()
{
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->proposalTableWidget);
...
{
    QStringList keys = XWCWallet::getInstance()->senatorProposalInfoMap.keys();
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        ProposalInfo info = XWCWallet::getInstance()->senatorProposalInfoMap.value(keys.at(i));
        ...
        ui->proposalTableWidget->setItem(i,1,new QTableWidgetItem(XWCWallet::getInstance()->guardAccountIdToName(info.proposer)));
        ...
        if(address.isEmpty())
        {
            ui->proposalTableWidget->setItem(i,4,new QTableWidgetItem(tr("no wallfacer")));
        }
        ...
        if(!address.isEmpty() && false == info.proposalFinished)          // 如果有wallfacer账户
        {
...
void ProposalPage::on_proposalTableWidget_cellClicked(int row, int column)
{
    ...
    if(column == 5)
    {
        ...
        QStringList senators = XWCWallet::getInstance()->getMyFormalGuards();
        if(senators.size() > 1)
        {
            CommonDialog commonDialog(CommonDialog::YesOrNo);
            commonDialog.setText(tr("There are %1 wallfacers in this wallet. Do you want all of them to approve?").arg(senators.size()));
            if(commonDialog.pop())
            {
                foreach (QString senator, senators)
                {
                    QString address = XWCWallet::getInstance()->accountInfoMap.value(senator).address;
                    array << address;
                }
            }
            else
        ...
        XWCWallet::getInstance()->postRPC( "id-approve_proposal", toJsonFormat( "approve_proposal",
