...
{
    ProposalInfo info;
    if(XWCWallet::getInstance()->senatorProposalInfoMap.contains(_proposalId))
    {
        info = XWCWallet::getInstance()->senatorProposalInfoMap.value(_proposalId);
    }
    else if(XWCWallet::getInstance()->citizenProposalInfoMap.contains(_proposalId))
    {
        info = XWCWallet::getInstance()->citizenProposalInfoMap.value(_proposalId);
    }
    else
    {
        return;
    ...
    {
        ui->voteStateTableWidget->setRowHeight(i,42);

        QString address = info.requiredAccounts.at(i);
        ui->voteStateTableWidget->setItem(i, 0, new QTableWidgetItem(QString("%1\n%2").
                                  arg(XWCWallet::getInstance()->guardOrCitizenAddressToName(address)).arg(address)));
    ...
    {
        ui->typeLabel->setText(tr("create wallfacer"));
    ...
    {
        ui->typeLabel->setText(tr("resign wallfacer"));
    ...
    {
        ui->typeLabel->setText(tr("change permanent wallfacer"));
        ...
        QMap<QString,GuardInfo> allGuard(XWCWallet::getInstance()->allGuardMap);
    ...
    case TRANSACTION_TYPE_CROSSCHAIN_FEE:
    {
        ...
        AssetInfo assetInfo = XWCWallet::getInstance()->assetInfoMap.value(XWCWallet::getInstance()->getAssetId(assetSymbol));
    ...
    {
        ui->typeLabel->setText(tr("set locked balance of wallfacer"));
...
QString ProposalDetailDialog::calProposalWeight(const ProposalInfo &info) const
{
    ...
    QMap<QString,MinerInfo> allMiner(XWCWallet::getInstance()->minerMap);
