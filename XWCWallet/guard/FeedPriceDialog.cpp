...
void FeedPriceDialog::init()
{
    ...
    QStringList keys = XWCWallet::getInstance()->accountInfoMap.keys();
    QStringList publishers;
    AssetInfo assetInfo = XWCWallet::getInstance()->assetInfoMap.value(XWCWallet::getInstance()->getAssetId( getRealAssetSymbol( ui->assetLabel->text())));
#ifdef SAFE_VERSION
    if(assetInfo.id.isEmpty())
    {
        assetInfo = XWCWallet::getInstance()->assetInfoMap.value(XWCWallet::getInstance()->getAssetId( "ERC" + ui->assetLabel->text()));
    }
#endif
    foreach (QString key, keys)
    {
        AccountInfo accountInfo = XWCWallet::getInstance()->accountInfoMap.value(key);
    ...
    if(publishers.contains(XWCWallet::getInstance()->currentAccount))
    {
...
void FeedPriceDialog::on_okBtn_clicked()
{
    ...
    AssetInfo baseAssetInfo = XWCWallet::getInstance()->assetInfoMap.value(XWCWallet::getInstance()->getAssetId( getRealAssetSymbol( ui->assetLabel->text())));
...
//    object.insert("core_exchange_rate", settlementPriceObject);
    ...
    XWCWallet::getInstance()->postRPC( "id-publish_normal_asset_feed", toJsonFormat( "publish_normal_asset_feed",
