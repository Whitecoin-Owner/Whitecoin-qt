...
{
    qDebug() << XWCWallet::getInstance()->senatorAutoWithdrawAmountMap;
    QStringList assets = XWCWallet::getInstance()->assetInfoMap.keys();
    assets.removeAll("1.3.0");
    int size = assets.size();
    ui->tableWidget->setRowCount(size);
    for(int i = 0; i < size; i++)
    {
        AssetInfo info = XWCWallet::getInstance()->assetInfoMap.value(assets.at(i));
        ...
        lineEdit->setText( QString::number( XWCWallet::getInstance()->getAssetAutoWithdrawLimit(info.symbol), 'g', 12));
    }
...
{
    XWCWallet::getInstance()->configFile->beginGroup("/AutoWithdrawAmount");
    for(int i = 0; i < ui->tableWidget->rowCount(); i++)
    {
        ...
        if(lineEdit)
        {
            XWCWallet::getInstance()->configFile->setValue(symbol, lineEdit->text().toDouble());
        }
    }
...
{
    save();
    XWCWallet::getInstance()->loadAutoWithdrawAmount();
    close();
}
