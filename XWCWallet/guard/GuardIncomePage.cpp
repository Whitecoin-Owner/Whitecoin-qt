...
void GuardIncomePage::init()
{
    ...
    else
    {
        ...
        ui->totalIncomeLabel->hide();
        ui->totalIncomeLabel2->hide();
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->senatorIncomeTableWidget);
...
void GuardIncomePage::fetchSenatorIncomes()
{
    ...
    XWCWallet::getInstance()->postRPC( "GuardIncomePage+get_address_pay_back_balance+" + ui->accountComboBox->currentText(),
...
void GuardIncomePage::showIncomes()
{
    ...
    {
        ui->senatorIncomeTableWidget->setRowHeight(i,42);
    ...
    ui->totalIncomeLabel->setText( getBigNumberString( totalAmount, ASSET_PRECISION) + " " + "XWC");
...
//        connect(feeCharge,&FeeChargeWidget::confirmSignal,[this,address,row](){
        ...
        if(commonDialog.pop())
        {
            ...
            object.insert("asset_id", XWCWallet::getInstance()->getAssetId(ASSET_NAME));
            array2 << object;
            array << array2;
            XWCWallet::getInstance()->postRPC( "GuardIncomePage+obtain_pay_back_balance",
