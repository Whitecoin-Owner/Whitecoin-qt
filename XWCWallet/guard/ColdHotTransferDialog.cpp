...
void ColdHotTransferDialog::init()
{
    ...
    foreach (QString assetId, assetIds)
    {
        ...
        ui->assetComboBox->addItem(XWCWallet::getInstance()->assetInfoMap.value(assetId).symbol, assetId);
    }
...
void ColdHotTransferDialog::on_okBtn_clicked()
{
    ...
    XWCWallet::getInstance()->postRPC( "id-transfer_from_cold_to_hot", toJsonFormat( "transfer_from_cold_to_hot",
...
{
    AssetInfo info = XWCWallet::getInstance()->assetInfoMap.value(XWCWallet::getInstance()->getAssetId(ui->assetComboBox->currentText()));
