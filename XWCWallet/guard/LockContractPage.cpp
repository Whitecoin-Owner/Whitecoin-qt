...
void LockContractPage::jsonDataUpdated(QString id)
{
    if( id == "LockContractPage+invoke_contract_offline+getUser+" + ui->accountComboBox->currentText())
    {
        ...
        if(result.startsWith("\"result\":"))
        {
            ...
            {
                ui->lockFundTableWidget->setRowHeight(i,42);
    ...
    if( id == "LockContractPage+invoke_contract_offline+getUsers")
    {
        ...
        if(result.startsWith("\"result\":"))
        {
            ...
            for(int i = 0; i < keys.size(); i++)
            {
                ...
                {
                    ui->lockFundTableWidget->setRowHeight(row,42);
                    ...
                    AssetInfo assetInfo = XWCWallet::getInstance()->assetInfoMap.value( XWCWallet::getInstance()->getAssetId(symbol));
                    ...
                    if(!XWCWallet::getInstance()->addressToName(address).isEmpty())
                    {
                        account = XWCWallet::getInstance()->addressToName(address);
                    }
                    else if(!XWCWallet::getInstance()->guardOrCitizenAddressToName(address).isEmpty())
                    {
                        account = XWCWallet::getInstance()->guardOrCitizenAddressToName(address);
                    }
...
void LockContractPage::getUserLockInfo(QString accountName)
{
    ...
    AccountInfo accountInfo = XWCWallet::getInstance()->accountInfoMap.value(accountName);
    XWCWallet::getInstance()->postRPC( "LockContractPage+invoke_contract_offline+getUser+" + accountName, toJsonFormat( "invoke_contract_offline",
...
{
    XWCWallet::getInstance()->postRPC( "LockContractPage+invoke_contract_offline+getUsers", toJsonFormat( "invoke_contract_offline",
                                                                           QJsonArray() << XWCWallet::getInstance()->accountInfoMap.keys().first()
