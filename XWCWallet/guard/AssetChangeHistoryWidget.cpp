...
void AssetChangeHistoryWidget::init()
{
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->changeHistoryTableWidget);
...
//        qDebug() << id << result;
        ...
        if(result.startsWith("\"result\":"))
        {
            ...
            {
                int row = size - i - 1;
                ui->changeHistoryTableWidget->setRowHeight(row,42);
                ...
                GuardMultisigAddress gma = XWCWallet::getInstance()->getGuardMultisigByPairId( getRealAssetSymbol( ui->assetComboBox->currentText()),
...
//                fetchCrosschainPrivateKey(gma.coldAddress);
                if(i == size - 1)
                {
                    XWCWallet::getInstance()->postRPC( "finish-dump_crosschain_private_key", toJsonFormat( "finish-dump_crosschain_private_key",
                                                                                                     QJsonArray() ));
                }
...
{
    XWCWallet::getInstance()->postRPC( "id-get_multisig_account_pair-" + getRealAssetSymbol( ui->assetComboBox->currentText()), toJsonFormat( "get_multisig_account_pair",
...
{
    XWCWallet::getInstance()->postRPC( "id-dump_crosschain_private_key-" + address, toJsonFormat( "dump_crosschain_private_key",
                                                                                     QJsonArray() << address));
}
