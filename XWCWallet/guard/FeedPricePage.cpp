...
void FeedPricePage::init()
{
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->assetPriceTableWidget);
...
void FeedPricePage::showAssetsPrice()
{
    ...
    {
        ui->assetPriceTableWidget->setRowHeight(i,42);

        AssetInfo info = XWCWallet::getInstance()->assetInfoMap.value(keys.at(i));

        ui->assetPriceTableWidget->setItem(i, 0, new QTableWidgetItem(info.symbol));
        ui->assetPriceTableWidget->setItem(i, 1, new QTableWidgetItem(toLocalTime(info.currentFeedTime)));
