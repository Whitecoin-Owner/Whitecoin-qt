...
void GuardKeyManagePage::init()
{
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->multisigTableWidget);
...
void GuardKeyManagePage::showMultisigInfo()
{
    ...
    assetIds.removeAll("1.3.0");    // 不显示XWC
    ...
    {
        ui->multisigTableWidget->setRowHeight(i,42);

        AssetInfo assetInfo = XWCWallet::getInstance()->assetInfoMap.value(assetIds.at(i));
        ...
        QStringList guardAccountIds = XWCWallet::getInstance()->getInstance()->getAssetMultisigUpdatedGuards(symbol);
        ...
        {
            ui->multisigTableWidget->setItem(i,5,new QTableWidgetItem(tr("no wallfacer")));
        }
        else
        {
            QString accountId = XWCWallet::getInstance()->allGuardMap.value(ui->accountComboBox->currentText()).accountId;
...
void GuardKeyManagePage::on_multisigTableWidget_cellClicked(int row, int column)
{
    ...
    if(column == 5)
    {
        ...
        {
            XWCWallet::getInstance()->postRPC( "id-update_asset_private_keys", toJsonFormat( "update_asset_private_keys",
...
void GuardKeyManagePage::on_importBtn_clicked()
{
    ...
    foreach (QString key, keys)
    {
        ...
        foreach (QJsonValue v, array)
        {
            ...
            XWCWallet::getInstance()->postRPC( "id-import_crosschain_key", toJsonFormat( "import_crosschain_key",
                                    QJsonArray()  << wifKey << key));
        }
    }

    XWCWallet::getInstance()->postRPC( "finish-import_crosschain_key", toJsonFormat( "import_crosschain_key",
                            QJsonArray() ));
}
