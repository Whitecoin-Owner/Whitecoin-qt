...
//        qDebug() << id << result;
    ...
    if( id == "ChangeCrosschainAddressDialog-wallfacer_sign_eths_multi_account_create_trx")
    {
        QString result = XWCWallet::getInstance()->jsonDataValue(id);
...
void ChangeCrosschainAddressDialog::on_okBtn_clicked()
{
    if(needSenatorSign)
    {
        ...
        {
            CommonDialog commonDialog(CommonDialog::OkOnly);
            commonDialog.setText(tr("You have no signer wallfacer in this wallet!"));
            commonDialog.pop();
            return;
        }
        ...
        if(!selectColdKeyFileDialog.filePath.isEmpty() && !selectColdKeyFileDialog.pwd.isEmpty())
        {
            ...
            XWCWallet::getInstance()->postRPC( "ChangeCrosschainAddressDialog-wallfacer_sign_eths_multi_account_create_trx", toJsonFormat( "wallfacer_sign_eths_multi_account_create_trx",
    ...
    else
    {
        ...
        XWCWallet::getInstance()->postRPC( "id-account_change_for_crosschain", toJsonFormat( "account_change_for_crosschain",
...
void ChangeCrosschainAddressDialog::on_assetComboBox_currentIndexChanged(const QString &arg1)
{
    ...
    {
        XWCWallet::getInstance()->postRPC( "ChangeCrosschainAddressDialog-get_eth_multi_account_trx-" + getRealAssetSymbol( ui->assetComboBox->currentText()),
...
{
    XWCWallet::getInstance()->postRPC( "ChangeCrosschainAddressDialog-get_multisig_account_pair-" + getRealAssetSymbol( ui->assetComboBox->currentText()),
...
{
    QStringList guards = XWCWallet::getInstance()->getMyFormalGuards();

    foreach (QString guard, guards)
    {
        AccountInfo info = XWCWallet::getInstance()->accountInfoMap.value(guard);
        XWCWallet::getInstance()->postRPC( "ChangeCrosschainAddressDialog-get_multi_address_obj-" + getRealAssetSymbol( ui->assetComboBox->currentText()) + "-" + info.id,
                                         toJsonFormat( "get_multi_address_obj", QJsonArray() << getRealAssetSymbol( ui->assetComboBox->currentText())
                                                                                << info.id));
    }

    XWCWallet::getInstance()->postRPC( "Finish-get_multi_address_obj-" + getRealAssetSymbol( ui->assetComboBox->currentText()),
