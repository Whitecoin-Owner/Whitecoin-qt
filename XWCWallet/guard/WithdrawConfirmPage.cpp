...
void WithdrawConfirmPage::init()
{
    ...
    QStringList assets = XWCWallet::getInstance()->getETHAssets();
    ...
    QStringList accounts = XWCWallet::getInstance()->getMyFormalGuards();
    if(accounts.size() > 0)
    {
    ...
    else
    {
        ...
        label->setGeometry(QRect(ui->label->pos(), QSize(280,30)));
        label->setText(tr("There are no wallfacer accounts in the wallet."));
        label->setStyleSheet(NOACCOUNT_TIP_LABEL);
    }


    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->crosschainTransactionTableWidget);
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->ethFinalTrxTableWidget);

    ui->typeCurrentBtn->setChecked(true);

    ui->autoSignCheckBox->setChecked(XWCWallet::getInstance()->autoWithdrawConfirm);
...
{
    if(id == "WithdrawConfirmPage-wallfacer_sign_crosschain_transaction")
    {
        ...
        {            
            ...
            commonDialog.setText(tr("Transaction of wallfacer-sign-crosschain has been sent out!"));
            commonDialog.pop();
        }
...
//            fetchCoinBalance(1014,ui->assetComboBox->currentText(),coldAddress);
    ...
    if(id == "id-wallfacer_sign_eths_final_trx")
    {
        QString result = XWCWallet::getInstance()->jsonDataValue(id);
        qDebug() << id << result;
        ...
        {
            CommonDialog commonDialog(CommonDialog::OkOnly);
            commonDialog.setText(tr("Transaction of wallfacer-sign-eth-withdraw-final has been sent out!"));
            commonDialog.pop();
        }
        else if(result.startsWith("\"error\":"))
        {
    ...
    if(column == 6)
    {
        if(currentType == 1)
        {
            if(ui->crosschainTransactionTableWidget->item(row,0) && ui->crosschainTransactionTableWidget->item(row,4))
            {
                ...
                QString generatedTrxId = XWCWallet::getInstance()->lookupGeneratedTrxByApplyTrxId(trxId);

                if(!generatedTrxId.isEmpty() && !ui->accountComboBox->currentText().isEmpty())
                {
                    XWCWallet::getInstance()->postRPC( "WithdrawConfirmPage-wallfacer_sign_crosschain_transaction", toJsonFormat( "wallfacer_sign_crosschain_transaction",
        ...
        else if(currentType == 0)
        {
            if(ui->crosschainTransactionTableWidget->item(row,0) && ui->crosschainTransactionTableWidget->item(row,2))
            {
                ...
                if(commonDialog.pop())
                {
                    ...
                    QString accountName = XWCWallet::getInstance()->addressToName(withdrawAddress);
                    if(!accountName.isEmpty())
                    {
                        XWCWallet::getInstance()->postRPC( "WithdrawConfirmPage-refund_request", toJsonFormat( "refund_request",
...
void WithdrawConfirmPage::showCrosschainTransactions()
{
    if(currentType == 1)
    {
        ...
        {
            ui->crosschainTransactionTableWidget->setRowHeight(i,42);

            ApplyTransaction at = XWCWallet::getInstance()->applyTransactionMap.value(keys.at(i));
    ...
    {
        QStringList keys = XWCWallet::getInstance()->pendingApplyTransactionMap.keys();
        int size = keys.size();
        ui->crosschainTransactionTableWidget->setRowCount(0);
        ui->crosschainTransactionTableWidget->setRowCount(size);
        ...
        for(int i = 0; i < size; i++)
        {
            ...
            ApplyTransaction at = XWCWallet::getInstance()->pendingApplyTransactionMap.value(keys.at(i));
            ...
            QString accountName = XWCWallet::getInstance()->addressToName(at.withdrawAddress);
            if(!accountName.isEmpty())
            {
...
void WithdrawConfirmPage::refreshCrosschainTransactionsState()
{
    ...
    for(int i = 0; i < rowCount; i++)
    {
        ...
        QString currentAddress = XWCWallet::getInstance()->accountInfoMap.value(ui->accountComboBox->currentText()).address;
...
void WithdrawConfirmPage::fetchEthBalance()
{
    ...
    XWCWallet::getInstance()->postRPC( "WithdrawConfirmPage-get_multi_address_obj-" + ui->assetComboBox->currentText() + "-" + accountInfo.id,
...
{
    QStringList keys = XWCWallet::getInstance()->ethFinalTrxMap.keys();
    int size = keys.size();
    ...
    for(int i = 0; i < size; i++)
    {
        ui->ethFinalTrxTableWidget->setRowHeight(i,42);

        ETHFinalTrx eft = XWCWallet::getInstance()->ethFinalTrxMap.value(keys.at(i));
        ...
        QString senatorName = XWCWallet::getInstance()->getGuardNameByHotColdAddress(eft.signer);
        ...
        XWCWallet::getInstance()->postRPC( "WithdrawConfirmPage+dump_crosschain_private_key+" + QString("%1+%2").arg(eft.signer).arg(i),
...
void WithdrawConfirmPage::on_ethFinalTrxTableWidget_cellClicked(int row, int column)
{
    if(column == 3)
    {
        if(ui->ethFinalTrxTableWidget->item(row,1))
        {
            ...
            XWCWallet::getInstance()->postRPC( "id-wallfacer_sign_eths_final_trx",
                                             toJsonFormat( "wallfacer_sign_eths_final_trx",
...
void WithdrawConfirmPage::on_autoSignCheckBox_stateChanged(int arg1)
{
    ...
    XWCWallet::getInstance()->autoWithdrawConfirm = !XWCWallet::getInstance()->autoWithdrawConfirm;
    XWCWallet::getInstance()->configFile->setValue("/settings/autoWithdrawConfirm", XWCWallet::getInstance()->autoWithdrawConfirm);
