...
void ColdHotTransferPage::init()
{
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->coldHotTransactionTableWidget);
...
void ColdHotTransferPage::jsonDataUpdated(QString id)
{
    ...
    if(id == "id-wallfacer_sign_coldhot_transaction")
    {
        QString result = XWCWallet::getInstance()->jsonDataValue(id);
        qDebug() << id << result;
        ...
        if(result.startsWith("\"result\":null"))
        {
            ...
            commonDialog.setText(tr("Transaction of wallfacer-sign-coldhot has been sent out!"));
            commonDialog.pop();
        }
    ...
    if(id == "id-wallfacer_sign_eths_coldhot_final_trx")
    {
        QString result = XWCWallet::getInstance()->jsonDataValue(id);
        qDebug() << id << result;
        ...
        {
            CommonDialog commonDialog(CommonDialog::OkOnly);
            commonDialog.setText(tr("Transaction of wallfacer-sign-eth-coldhot-final has been sent out!"));
            commonDialog.pop();
        }
        else if(result.startsWith("\"error\":"))
        {
...
{
    XWCWallet::getInstance()->postRPC( "id-get_coldhot_transaction-" + QString::number(0), toJsonFormat( "get_coldhot_transaction",
                                                                                     QJsonArray() << 0));

    XWCWallet::getInstance()->postRPC( "id-get_coldhot_transaction-" + QString::number(1), toJsonFormat( "get_coldhot_transaction",
                                                                                     QJsonArray() << 1));

    XWCWallet::getInstance()->postRPC( "id-get_coldhot_transaction-" + QString::number(2), toJsonFormat( "get_coldhot_transaction",
                                                                                     QJsonArray() << 2));

    XWCWallet::getInstance()->postRPC( "id-get_coldhot_transaction-" + QString::number(6), toJsonFormat( "get_coldhot_transaction",
                                                                                     QJsonArray() << 6));
...
void ColdHotTransferPage::showColdHotTransactions()
{
    if(currentType == 1)
    {
        ...
        {
            ui->coldHotTransactionTableWidget->setRowHeight(i,42);
...
void ColdHotTransferPage::fetchEthBalance()
{
    ...
    XWCWallet::getInstance()->postRPC( "ColdHotTransferPage-get_multi_address_obj-" + ui->assetComboBox->currentText() + "-" + accountInfo.id,
...
//        XWCWallet::getInstance()->postRPC( "ColdHotTransferPage+dump_crosschain_private_key+" + QString("%1+%2").arg(eft.signer).arg(i),
...
//            if(!checkPwdDialog.pop())   return;
                    ...
                    && ui->coldHotTransactionTableWidget->item(row,3))
            {
                ...
                if(!chcTrxId.isEmpty() && !ui->accountComboBox->currentText().isEmpty())
                {
                    ...
                    AssetInfo assetInfo = XWCWallet::getInstance()->assetInfoMap.value(cht.assetId);
                    if(assetInfo.coldAddress == cht.withdrawAddress)
                    {
                        ...
                        {
                            XWCWallet::getInstance()->postRPC( "id-wallfacer_sign_coldhot_transaction", toJsonFormat( "wallfacer_sign_coldhot_transaction",
...
void ColdHotTransferPage::on_ethFinalTrxTableWidget_cellClicked(int row, int column)
{
    if(column == 3)
    {
        if(ui->ethFinalTrxTableWidget->item(row,1))
        {
            ...
            {
                XWCWallet::getInstance()->postRPC( "id-wallfacer_sign_eths_coldhot_final_trx",
                                                 toJsonFormat( "wallfacer_sign_eths_coldhot_final_trx",
