...
void checkAndLogMd5(const QString &nodePath,const QString &nodeMd5,const QString &clientPath,const QString &clientMd5){
    ...
    if(node_md5 != nodeMd5)
    {
        logToFile( QStringList() <<"WARNING: xwc_node md5 mismatching. current:"<<node_md5<<" standard:"<<nodeMd5);
    }
    else
    {
        logToFile( QStringList() <<"LOG: xwc_node md5 matched:"<<node_md5);
    }
    if(client_md5 != clientMd5)
    {
        logToFile( QStringList() <<"WARNING: xwc_client md5 mismatching. current:"<<client_md5<<" standard:"<<clientMd5);
    }
    else
    {
        logToFile( QStringList() <<"LOG: xwc_client md5 matched:"<<client_md5);
    }
}
