...
#ifdef TEST_WALLET
    //  创建互斥量
    HANDLE m_hMutex  =  CreateMutex(NULL, FALSE,  L"XWCWallet_test" );
#else
    //  创建互斥量
    HANDLE m_hMutex  =  CreateMutex(NULL, FALSE,  L"XWCWallet" );
#endif
    //  检查错误代码
...
bool checkOnly()
{
    const char filename[] = "/tmp/XWCWalletlockfile";
...
void myMessageOutput(QtMsgType type, const QMessageLogContext &context, const QString &msg)
{
    ...
    QFileInfo info("xwc_log.txt");
    if(info.size() > 1024*1024*50) QFile::remove("xwc_log.txt");
    QFile file("xwc_log.txt");
...
//    a.installTranslator(&translator);
    XWCWallet::getInstance();  // 在frame创建前先创建实例，读取language
    qDebug() <<  "db init: " << XWCWallet::getInstance()->transactionDB.init();
//    XWCWallet::getInstance()->witnessConfig->append("trackaddress", "axiba",  true);
//    XWCWallet::getInstance()->witnessConfig->modify("crosschain-ip", "192.168.1.111");
//    XWCWallet::getInstance()->witnessConfig->remove("crosschain-port");
//    qDebug() << XWCWallet::getInstance()->witnessConfig->value("private-key");
//    XWCWallet::getInstance()->witnessConfig->addPrivateKey("GGGGGGG", "BBBBBBBBB");
//    XWCWallet::getInstance()->witnessConfig->addMiner("1.6.27");
//    XWCWallet::getInstance()->witnessConfig->addMiner("1.6.25");
//    qDebug() << XWCWallet::getInstance()->witnessConfig->getMiners();
//     XWCWallet::getInstance()->witnessConfig->save();

//    QStringList keys = XWCWallet::getInstance()->transactionDB.keys();
...
//    removeUpdateDeleteFile();
    ...
    Frame frame;
    XWCWallet::getInstance()->mainFrame = &frame;   // 一个全局的指向主窗口的指针
    frame.show();
