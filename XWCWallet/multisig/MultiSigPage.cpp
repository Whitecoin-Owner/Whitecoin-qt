...
//    ui->infoTableWidget->horizontalHeader()->setFixedHeight(40);
    ...
    ui->deleteBtn->setStyleSheet(TOOLBUTTON_STYLE_1);
    ui->addMultiSigBtn->setStyleSheet(TOOLBUTTON_STYLE_1);
    ui->createMultiSigBtn->setStyleSheet(TOOLBUTTON_STYLE_1);
...
{
    XWCWallet::getInstance()->postRPC( "MultiSigPage-get_multisig_address-" + ui->multiSigAddressComboBox->currentText(),
...
void MultiSigPage::showMultiSigInfo()
{
    ...
    {
        ui->infoTableWidget->setRowHeight(i,42);

        ui->infoTableWidget->setItem( i, 0, new QTableWidgetItem( info.addresses.at(i)));

        QString accountName = XWCWallet::getInstance()->addressToName(info.addresses.at(i));
...
{
    XWCWallet::getInstance()->postRPC( "MultiSigPage-get_addr_balances-" + ui->multiSigAddressComboBox->currentText(),
...
void MultiSigPage::showBalances()
{
    ...
    {
        ui->balanceTableWidget->setRowHeight(i,42);
...
//    }
    ...
    if( id.startsWith("SignMultiSigTrx+get_multisig_address"))
    {
        ...
        foreach (QJsonValue v, array2)
        {
            QString account = XWCWallet::getInstance()->addressToName(v.toString());
            if(!account.isEmpty())
            {
    ...
    if( id == "MultiSigPage+decode_multisig_transaction")
    {
        ...
        if( result.startsWith("\"result\":"))
        {
            ...
            XWCWallet::getInstance()->postRPC( "SignMultiSigTrx+get_multisig_address",
    ...
    if( id == "MultiSigPage+sign_multisig_trx")
    {
        ...
        if( result.startsWith("\"result\":"))
        {
            ...
            XWCWallet::getInstance()->postRPC( "SignedTrx+decode_multisig_transaction",
...
void MultiSigPage::init()
{
    ...
    XWCWallet::getInstance()->configFile->beginGroup("/multisigAddresses");
...
void MultiSigPage::paintEvent(QPaintEvent *)
{
    ...
    painter.drawRect(0,67,this->width(),24);
}
...
//    XWCWallet::getInstance()->postRPC( "MultiSigPage-get_pubkey_from_account-" + ui->accountComboBox->currentText(),
...
void MultiSigPage::on_trxCodeTextBrowser_textChanged()
{
    ...
    {
        XWCWallet::getInstance()->postRPC( "MultiSigPage+decode_multisig_transaction",
                                         toJsonFormat( "decode_multisig_transaction",
...
{
    XWCWallet::getInstance()->postRPC( "MultiSigPage+sign_multisig_trx",
...
{
    XWCWallet::getInstance()->postRPC( "MultiSigPage+combine_transaction",
...
void MultiSigPage::on_addMultiSigBtn_clicked()
{
    ...
    {
        XWCWallet::getInstance()->configFile->setValue("/multisigAddresses/" + addMultiSigAddressDialog.multiSigAddress, 1);
        init();

#ifndef LIGHT_MODE
        XWCWallet::getInstance()->witnessConfig->addTrackAddress(addMultiSigAddressDialog.multiSigAddress);
...
void MultiSigPage::on_deleteBtn_clicked()
{
    ...
    if(commonDialog.pop())
    {
        XWCWallet::getInstance()->configFile->remove("/multisigAddresses/" + ui->multiSigAddressComboBox->currentText());
        init();
    }
}
