...
//    ui->accountTableWidget->horizontalHeader()->setFixedHeight(40);
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->accountTableWidget);
...
void MainPage::updateAccountList()
{
    ...
    {
        ui->accountTableWidget->setRowHeight(i,42);
...
void MainPage::on_addAccountBtn_clicked()
{
    ...
    if( !name.isEmpty())
    {
        XWCWallet::getInstance()->postRPC( "id-wallet_create_account-" + name, toJsonFormat( "wallet_create_account", QJsonArray() << name ));
