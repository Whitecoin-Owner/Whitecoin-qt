...
void PoundageShowWidget::DeletePoundageSlots()
{
    emit DeletePoundageSignal(ui->tableView->currentIndex().data(Qt::UserRole).value<std::shared_ptr<PoundageUnit>>()->poundageID
                              ,XWCWallet::getInstance()->addressToName(ui->tableView->currentIndex().data(Qt::UserRole).value<std::shared_ptr<PoundageUnit>>()->ownerAdress));
}
...
//    ui->tableView->hideColumn(2);
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->tableView);
}
