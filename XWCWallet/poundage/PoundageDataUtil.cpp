...
bool PoundageDataUtil::ParseJsonObjToUnit(QJsonObject jsonObj,std::shared_ptr<PoundageUnit>& poundageUnit)
{
    ...
    int oriPre = XWCWallet::getInstance()->assetInfoMap.value(oriObj.value("asset_id").toString()).precision;
    ...
    int tarPre = XWCWallet::getInstance()->assetInfoMap.value(tarObj.value("asset_id").toString()).precision;
