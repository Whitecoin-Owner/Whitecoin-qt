...
void CitizenProposalPage::jsonDataUpdated(QString id)
{
    ...
    else if("query_object_state"==id)
    {
        ...
        isCurrentTimeBigThanNextVoteTime = QDateTime::fromString(XWCWallet::getInstance()->currentBlockTime,"yyyy-MM-ddThh:mm:ss") >
...
{
    //查询当前vote状态
    XWCWallet::getInstance()->postRPC("query_object_state",toJsonFormat("get_object",QJsonArray()<<"2.1.0"));
...
void CitizenProposalPage::on_proposalTableWidget_cellClicked(int row, int column)
{
    ...
    if(column == 6)
    {
        ...
        if(commonDialog.pop())
        {
            ...
            XWCWallet::getInstance()->postRPC( "id-approve_proposal", toJsonFormat( "approve_referendum",
