...
void ChangePayBackDialog::InitWidget()
{
    ...
    connect(XWCWallet::getInstance(),&XWCWallet::jsonDataUpdated,this,&ChangePayBackDialog::jsonDataUpdated);


    FeeChooseWidget *feeWidget = new FeeChooseWidget(XWCWallet::getInstance()->feeChargeInfo.changePaybackFee.toDouble(),
                                                     XWCWallet::getInstance()->feeType,citizenName);
...
void ChangePayBackDialog::InitData()
{
    ...
    foreach (QString info, citizens) {
        QMap<QString,MinerInfo> miners(XWCWallet::getInstance()->minerMap);
