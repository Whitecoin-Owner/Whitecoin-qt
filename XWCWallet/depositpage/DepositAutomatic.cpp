...
public:
    DataPrivate()
        :autoDeposit(XWCWallet::getInstance()->autoDeposit)
        ,isInUpdate(false)
    {
    ...
    ,_p(new DataPrivate())
{
    ...
    connect( XWCWallet::getInstance(), &XWCWallet::jsonDataUpdated, this, &DepositAutomatic::jsonDataUpdated);
...
{
    _p->autoDeposit = XWCWallet::getInstance()->autoDeposit;
    if(_p->autoDeposit && !_p->isInUpdate && XWCWallet::getInstance()->GetBlockSyncFinish())
    {
        //_p->timer->stop();
        updateData();
    }
...
void DepositAutomatic::PostQueryTunnelAddress(const QString &accountName, const QString &symbol)
{
    ...
    XWCWallet::getInstance()->postRPC( "automatic-get_binding_account-"+accountName,
...
void DepositAutomatic::PostQueryMultiAddress(const QString &symbol)
{
    ...
    XWCWallet::getInstance()->postRPC( "automatic-get_current_multi_address_"+symbol,
...
//    }
    ...
    XWCWallet::getInstance()->postRPC( "automatic-createrawtransaction_"+fromAddress,
                                          ...
                                          const QJsonObject &detail)
{
    ...
    XWCWallet::getInstance()->postRPC( "automatic-signrawtransaction",
...
{
    XWCWallet::getInstance()->postRPC( "automatic-finish-tunnel",
...
{
    XWCWallet::getInstance()->postRPC( "automatic-finish-money",
...
{
    XWCWallet::getInstance()->postRPC( "automatic-finish-multi",
...
{
    XWCWallet::getInstance()->postRPC( "automatic-finish-transaction",
...
{
    XWCWallet::getInstance()->postRPC( "automatic-finish-sign",
...
void DepositAutomatic::updateData()
{
    ...
    QMap<QString,AccountInfo> accountInfoMap = XWCWallet::getInstance()->accountInfoMap;
    QMap<QString,AssetInfo> assetInfoMap = XWCWallet::getInstance()->assetInfoMap;
