...
void DepositPage::jsonDataUpdated(QString id)
{
    ...
    //提取通道账户地址
        ...
        {
#ifndef LIGHT_MODE
            if(!XWCWallet::getInstance()->witnessConfig->getChainTypes().contains(_p->assetSymbol))
            {
            ...
            {
                QString tunn = XWCWallet::getInstance()->configFile->value(QString("/tunnel/%1%2").arg(_p->address).arg(_p->actualAssetSymbol)).toString();
                //如果本地没有缓存，则新建，否则生成一个
                if(tunn.isEmpty())
                {
                    //生成通道账户
...
//        qDebug() << id << result;
    ...
    else if(id.startsWith("deposit_bind_tunnel_account_"))
    {
        ...
        XWCWallet::getInstance()->configFile->setValue(QString("/tunnel/%1%2").arg(_p->address).arg(_p->actualAssetSymbol),_p->tunnelData->address);
...
{
    XWCWallet::getInstance()->postRPC( "deposit_get_binding_account",
...
{
    XWCWallet::getInstance()->postRPC( "deposit_get_actual_binding_account",
                                     toJsonFormat( "get_binding_account", QJsonArray()
...
{
    if(!XWCWallet::getInstance()->ValidateOnChainOperation())
    {
    ...
    XWCWallet::getInstance()->postRPC("deposit_create_crosschain_symbol",
...
void DepositPage::BindTunnelAccount()
{
    ...
    XWCWallet::getInstance()->postRPC("deposit_bind_tunnel_account_"+_p->assetSymbol,
...
void DepositPage::BindActualTunnelAccount()
{
    ...
    XWCWallet::getInstance()->postRPC("deposit_bind_tunnel_account_"+_p->actualAssetSymbol,
                                        toJsonFormat("bind_tunnel_account",
...
{//开始查找私钥
    XWCWallet::getInstance()->postRPC( "deposit-dump_crosschain_private_keys", toJsonFormat( "dump_crosschain_private_keys", QJsonArray()));
...
void DepositPage::InitWidget()
{
    ...
    connect( XWCWallet::getInstance(), &XWCWallet::jsonDataUpdated, this, &DepositPage::jsonDataUpdated);


    _p->fee = new FeeChargeWidget(XWCWallet::getInstance()->feeChargeInfo.tunnelBindFee.toDouble(),ASSET_NAME,_p->name,
