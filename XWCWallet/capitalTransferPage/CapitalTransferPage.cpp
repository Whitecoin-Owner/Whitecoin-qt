...
//    qDebug()<<"pppppppppppp"<<_p->actualNumber;
    CapitalConfirmWidget *confirmWidget = new CapitalConfirmWidget(CapitalConfirmWidget::CapitalConfirmInput(
                                                                   ...
                                                                   actualShow,feeShow,totalShow),XWCWallet::getInstance()->mainFrame);
...
//        ui->toolButton_confirm->setVisible(true);
    ...
    else if("captial-signrawtransaction" == id)
    {
        ...
        qDebug()<<"signature"<<XWCWallet::getInstance()->jsonDataValue( id);

        if( result.startsWith("{\"result\":"))             // 成功
        {
            XWCWallet::getInstance()->mainFrame->crossMark->TransactionInput(result,_p->symbol,_p->account_name,ui->lineEdit_number->text().toDouble());
...
//        }
    ...
    {//充值修改，发送充值指令问题
        ...
        XWCWallet::getInstance()->postRPC( "captial-createrawtransaction",
...
void CapitalTransferPage::getMarkNumber()
{
    ...
    double number = XWCWallet::getInstance()->mainFrame->crossMark->CalTransaction(_p->account_name,_p->symbol);
...
//        ui->label_fee->setText( tr("about") + " 0.00045 ETH");
    ...
    connect( XWCWallet::getInstance(), &XWCWallet::jsonDataUpdated, this, &CapitalTransferPage::jsonDataUpdated);
    ...
    //记录标志
    ui->label_marktip->setVisible(false);
    connect(XWCWallet::getInstance()->mainFrame->crossMark,&CrossCapitalMark::updateMark,this,&CapitalTransferPage::getMarkNumber);

    //获取通道账户，获取多签地址，用于充值使用
    XWCWallet::getInstance()->postRPC( "captial-get_binding_account",
                                     toJsonFormat( "get_binding_account", QJsonArray()
                                     << _p->account_name<<_p->symbol ));
    XWCWallet::getInstance()->postRPC( "captial-get_current_multi_address",
                                     toJsonFormat( "get_current_multi_address", QJsonArray()
                                     << _p->symbol));
    {
        XWCWallet::getInstance()->mainFrame->crossMark->checkUpData(_p->account_name,_p->symbol);
    }
