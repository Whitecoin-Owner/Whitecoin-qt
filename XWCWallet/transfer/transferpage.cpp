...
//            {
//                XWCWallet::getInstance()->postRPC( "id-set_guarantee_id",
...
//            }
            emit usePoundage();
            XWCWallet::getInstance()->postRPC( "id-transfer_to_address-" + accountName,
...
//    feeID = XWCWallet::getInstance()->configFile->value("/settings/feeOrderID").toString();
//    if(feeID.isEmpty())
//    {
...
//    {
//        //查询承兑单
//        XWCWallet::getInstance()->postRPC( "id-get_guarantee_order",
...
//        ui->tipLabel4->show();
        ...
        ui->tipLabel4->show();
        XWCWallet::getInstance()->postRPC( "TransferPage+get_account+" + ui->sendtoLineEdit->text(),
...
void TransferPage::on_transferRecordBtn_clicked()
{
    ...
    static_cast<TransferRecordWidget*>(currentTopWidget)->showTransferRecord(XWCWallet::getInstance()->accountInfoMap.value(accountName).address);
}
...
{//根据当前账户、资产类型，判断最大转账数量
    ...
    QString assetID ;
    int assetPrecision ;
    foreach (AssetInfo in, XWCWallet::getInstance()->assetInfoMap) {
    ...
    QMapIterator<QString, AccountInfo> i(XWCWallet::getInstance()->accountInfoMap);
      while (i.hasNext()) {
          i.next();
          ...
          while(am.hasNext()){
              ...
              if(am.key() == assetID)
              {
                  AssetInfo assetInfo = XWCWallet::getInstance()->assetInfoMap.value(am.value().assetId);
