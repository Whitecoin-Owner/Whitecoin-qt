...
//    ui->transferRecordTableWidget->setColumnWidth(5,70);
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->transferRecordTableWidget);
    init();
}
...
void TransferRecordWidget::showTransferRecord(QString _accountAddress, QString _assetId)
{
    ...
    TransactionTypeIds typeIds = XWCWallet::getInstance()->transactionDB.getAccountTransactionTypeIdsByType(_accountAddress,TRANSACTION_TYPE_NORMAL);
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        ui->transferRecordTableWidget->setRowHeight(rowCount,42);
