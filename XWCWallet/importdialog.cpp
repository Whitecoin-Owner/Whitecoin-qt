...
void ImportDialog::on_importBtn_clicked()
{
    ...
    {
        // 如果是XWC私钥
        XWCWallet::getInstance()->postRPC( "import-import_key",
    ...
    else if(QFile(ui->privateKeyLineEdit->text()).exists())
    {
        ...
        //验证该地址是否已经存在
        foreach(AccountInfo info,XWCWallet::getInstance()->accountInfoMap){
            qDebug()<<info.address << " "<<data->XWCAddr;
            if(info.address == data->XWCAddr)
            {
                CommonDialog commonDialog(CommonDialog::OkOnly);
                commonDialog.setText( tr("XWC Address:%1  Already Exists!").arg(data->XWCAddr));
                commonDialog.pop();
                return;
            }
        }
        ...
        for(auto it = data->info_key.begin();it != data->info_key.end();++it)
        {
            ...
            else
            {
                if((*it).first.contains("-"))
                {
                    ...
                    XWCWallet::getInstance()->postRPC( "import-import_crosschain_key",
        ...
        XWCWallet::getInstance()->postRPC( "import-finish_import_key", toJsonFormat( "finish_import_key", QJsonArray()));
...
//            XWCWallet::getInstance()->postRPC( "id-import_key", toJsonFormat( "import_key", QJsonArray() << ui->accountNameLineEdit->text() << str));
//            ui->importBtn->setEnabled(false);
//            shadowWidget->show();
...
//                    XWCWallet::getInstance()->postRPC( "id-import_key", toJsonFormat( "import_key", QJsonArray() << ui->accountNameLineEdit->text() << pk));
//                    ui->importBtn->setEnabled(false);
//                    shadowWidget->show();
//                }
//                else
...
//        {
//            XWCWallet::getInstance()->postRPC( "id-import_key", toJsonFormat( "import_key", QJsonArray() << ui->accountNameLineEdit->text() << privateKey));
...
//            emit accountImported();
            ...
            XWCWallet::getInstance()->postRPC( "id-get_account-ImportDialog-" + ui->accountNameLineEdit->text(), toJsonFormat( "get_account", QJsonArray() << ui->accountNameLineEdit->text() ));
...
#ifndef LIGHT_MODE
        XWCWallet::getInstance()->witnessConfig->addTrackAddress(accountAddress);
