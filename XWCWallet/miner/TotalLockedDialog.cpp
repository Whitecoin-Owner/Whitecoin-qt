    ...
    ui(new Ui::TotalLockedDialog)
{
    ...
    setStyleSheet("#totalLockedDialog{border:1px solid rgb(173,173,173);background-color:white;}");
...
void TotalLockedDialog::calculateTotalLocked()
{
    ...
    QStringList keys = XWCWallet::getInstance()->minerMap.keys();
    foreach (QString key, keys)
    {
        MinerInfo info = XWCWallet::getInstance()->minerMap.value(key);
