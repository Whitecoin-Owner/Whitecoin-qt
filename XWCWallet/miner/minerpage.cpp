...
//    ui->citizenInfoTableWidget->horizontalHeader()->setFixedHeight(30);
    ...
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->lockBalancesTableWidget);
    XWCWallet::getInstance()->mainFrame->installBlurEffect(ui->citizenInfoTableWidget);
    InitStyle();
    init();
}
...
void MinerPage::jsonDataUpdated(QString id)
{
    ...
    if( id == "id-get_address_pay_back_balance-" + ui->accountComboBox->currentText())
    {
        ...
        for(int i = 0; i < size; i++)
        {
            ui->incomeTableWidget->setRowHeight(i,42);
...
void MinerPage::on_registerBtn_clicked()
{
    ...
    if(XWCWallet::getInstance()->getUnregisteredAccounts().isEmpty())
    {
...
void MinerPage::paintEvent(QPaintEvent *)
{
    ...
    painter.drawRect(0,62,this->width(),24);
}
...
void MinerPage::updateAccounts()
{
    ...
    if( XWCWallet::getInstance()->getRegisteredAccounts().isEmpty())
    {
        ui->tipLabel->show();
    ...
    else
    {
        ...
        if(keys.contains(XWCWallet::getInstance()->currentAccount))
        {
...
{
    XWCWallet::getInstance()->postRPC( "id-get_account_lock_balance-" + ui->accountComboBox->currentText(),
...
void MinerPage::fetchAccountIncome()
{
    ...
    XWCWallet::getInstance()->postRPC( "id-get_address_pay_back_balance-" + ui->accountComboBox->currentText(),
...
{
    QString address = XWCWallet::getInstance()->accountInfoMap.value(ui->accountComboBox->currentText()).address;
    TransactionTypeIds typeIds = XWCWallet::getInstance()->transactionDB.getAccountTransactionTypeIdsByType(address,TRANSACTION_TYPE_MINE_INCOME);
    ...
    for(int i = 0; i < typeIds.size(); i++)
    {
        ...
        TransactionStruct ts = XWCWallet::getInstance()->transactionDB.getTransactionStruct(typeIds.at(i).transactionId);
    ...
    for(int i = 0; i < size; i++)
    {
        ui->incomeRecordTableWidget->setRowHeight(i,42);
...
{
    QStringList citizens = XWCWallet::getInstance()->minerMap.keys();
    ...
    for(int i = 0; i < citizens.size(); i++)
    {
        ...
        {
            if(XWCWallet::getInstance()->minerMap.value(sortedKeysByPledgeWeight.at(j)).pledgeWeight
                    < XWCWallet::getInstance()->minerMap.value(citizens.at(i)).pledgeWeight)
            {
    ...
    for(int i = 0; i < sortedKeysByPledgeWeight.size(); i++)
    {
        ...
        case 0:
            ...
            {
                if(XWCWallet::getInstance()->minerMap.value(sortedKeys.at(j)).pledgeWeight
                        < XWCWallet::getInstance()->minerMap.value(sortedKeysByPledgeWeight.at(i)).pledgeWeight)
                {
        ...
        case 1:
            for(int j = 0; j < sortedKeys.size(); j++)
            {
                if(XWCWallet::getInstance()->minerMap.value(sortedKeys.at(j)).totalProduced
                        < XWCWallet::getInstance()->minerMap.value(sortedKeysByPledgeWeight.at(i)).totalProduced)
                {
                    sortedKeys.insert( j, sortedKeysByPledgeWeight.at(i));
                    break;
                }
        ...
        case 2:
            for(int j = 0; j < sortedKeys.size(); j++)
            {
                if(XWCWallet::getInstance()->minerMap.value(sortedKeys.at(j)).totalMissed
                        > XWCWallet::getInstance()->minerMap.value(sortedKeysByPledgeWeight.at(i)).totalMissed)
                {
                    sortedKeys.insert( j, sortedKeysByPledgeWeight.at(i));
                    break;
                }
        ...
        case 3:
            for(int j = 0; j < sortedKeys.size(); j++)
            {
                if(XWCWallet::getInstance()->minerMap.value(sortedKeys.at(j)).payBack
                        > XWCWallet::getInstance()->minerMap.value(sortedKeysByPledgeWeight.at(i)).payBack)
                {
                    sortedKeys.insert( j, sortedKeysByPledgeWeight.at(i));
                    break;
                }
...
{
    AccountInfo accountInfo = XWCWallet::getInstance()->accountInfoMap.value(ui->accountComboBox->currentText());
    unsigned long long amount = accountInfo.assetAmountMap.value( XWCWallet::getInstance()->getAssetId(ASSET_NAME)).amount;
...
void MinerPage::autoObtain()
{
    ...
    for(int i = 0; i < ui->incomeTableWidget->rowCount(); i++)
    {
        ...
        object.insert("asset_id", XWCWallet::getInstance()->getAssetId(amountStringList.at(1)));
        array2 << object;
        array << array2;
    }

    XWCWallet::getInstance()->postRPC( "MinerPage-obtain_pay_back_balance",
...
//        FeeChargeWidget *feeCharge = new FeeChargeWidget(XWCWallet::getInstance()->feeChargeInfo.minerIncomeFee.toDouble(),
//                                                         XWCWallet::getInstance()->feeType,ui->accountComboBox->currentText(),
//                                                         XWCWallet::getInstance()->mainFrame);
//        connect(feeCharge,&FeeChargeWidget::confirmSignal,[this,address,row](){

        CommonDialog commonDialog(CommonDialog::YesOrNo);
        commonDialog.setText(tr("You need to pay %1 %2 for this operation. Continue?").arg(XWCWallet::getInstance()->feeChargeInfo.minerIncomeFee).arg(ASSET_NAME));
        if(commonDialog.pop())
        {
            QJsonArray array;
            QJsonArray array2;
            ...
            array2 << object;
            array << array2;
            XWCWallet::getInstance()->postRPC( "id-obtain_pay_back_balance",
                                             toJsonFormat( "obtain_pay_back_balance",
                                                           QJsonArray() << address
                                                           << array
                                                           << true ));
...
{
    if(!XWCWallet::getInstance()->minerMap.contains(citizenName)) return "";
    MinerInfo citizenInfo = XWCWallet::getInstance()->minerMap.value(citizenName);
